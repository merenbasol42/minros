...
/// @file logger.hpp
/// @brief logging — minros log overlay'i (Logger yayıncı + LogSink alıcı).
///
/// @ref minros::overlays::reliability::Reliable gibi bağımsız bir overlay:
/// core'a hiçbir şey eklemez, yalnızca RawNode'un public publish/subscribe
/// API'sini kullanır. Log seviyesini + parça bilgisini payload'ın önüne 1
/// baytlık opak FLAGS öneki olarak koyar ve rezerve LOG kanalından (CH248) yayar.
///
/// İki ayrı sınıf — publisher ve sink'i ayırmak RAM kısıtlı slave'i korur:
///   - @ref minros::overlays::logging::Logger "Logger" (publisher, slave): SIFIR buffer tutar. Log'lar
///     fire-and-forget olduğundan payload'ı saklamaz; string literal zaten
///     flash'ta yaşar ve framer oradan byte-byte okur (memory-mapped flash
///     hedeflerinde kopya yok). Kaynakta seviye filtresi ile min_level altı
///     çağrılar wire'a hiç dokunmadan döner.
///   - @ref minros::overlays::logging::LogSink "LogSink" (subscriber, host): parçaları yeniden birleştirir. Yalnızca
///     burada bir reassembly buffer (REASM_BUF) vardır. Slave bunu instantiate
///     etmez → maliyet bindirmez.
///
/// @warning Bu tasarım flash'ı memory-mapped olan hedefleri varsayar
///          (Cortex-M/STM32, ESP32). Klasik AVR (Harvard) mimarisinde flash
///          memory-mapped DEĞİLDİR: string literal ya başlangıçta RAM'e
///          kopyalanır (RAM israfı), ya da PROGMEM + `pgm_read_byte` gerekir;
///          framer'ın düz `payload[i]` okuması bunu yapamaz. Dolayısıyla
///          AVR'de bu logger (özellikle flash'tan zero-copy fragmentation)
///          olduğu gibi ÇALIŞMAZ. AVR desteği gerekirse PROGMEM-aware ayrı
///          bir okuma yolu tasarlanmalıdır.
///
/// @code
/// // Kullanım (slave):
/// RawNode<> node;
...
/// @brief Yayıncı — zero-buffer, slave tarafı.
/// @tparam NodeT      Taşıyıcı node tipi (publish gerektirir).
/// @tparam FRAME_DATA Node'un frame DATA bütçesi (CH_ID + head + payload).
///                    Node'un `MAX_FRAME_DATA`'sıyla eşleşmeli; parça boyutu
...
public:
    ...
    /// @brief Kaynakta seviye filtresini ayarlar: altındaki çağrılar wire'a dokunmadan döner.
    void  set_min_level(Level l) { min_ = l; }
    /// @brief Geçerli minimum log seviyesi.
    Level min_level() const      { return min_; }

    /// @brief Ham byte log (binary-safe). Uzunsa CHUNK'lara bölünüp SEQ4 ile numaralanır.
    /// @param l   Log seviyesi.
    ...
    /// @brief C-string kolaylığı — pointer flash'ta olabilir, kopya yok.
    void log(Level l, const char* s) { log(l, reinterpret_cast<const u8*>(s), cstrlen(s)); }
    /// @brief DEBUG seviyesinde C-string log.
    void debug(const char* s) { log(Level::DEBUG, s); }
    /// @brief INFO seviyesinde C-string log.
    void info (const char* s) { log(Level::INFO,  s); }
    /// @brief WARN seviyesinde C-string log.
    void warn (const char* s) { log(Level::WARN,  s); }
    /// @brief ERROR seviyesinde C-string log.
    void error(const char* s) { log(Level::ERROR, s); }
    /// @brief FATAL seviyesinde C-string log.
...
/// @brief Sink — reassembler, host tarafı.
/// @tparam NodeT     Taşıyıcı node tipi (subscribe gerektirir).
/// @tparam REASM_BUF Birleştirme buffer'ı; en uzun beklenen log satırı kadar.
template<class NodeT, u8 REASM_BUF = 128>
class LogSink {
public:
    /// @brief Log callback imzası: `fn(level, msg, len, ctx)` — birleştirilmiş
    ///        tam satır, FLAGS ayıklanmış.
    ...
    /// @brief LOG kanalına abone olur.
    /// @param cb Birleştirilmiş log satırı geldiğinde çağrılacak callback.
    /// @return Node'un abonelik slotu doluysa `false`.
    bool subscribe(LogCallback cb) {
        cb_ = cb;
        return node_->subscribe(protocol::LOG_CHANNEL_ID, {&LogSink::rx_thunk, this});
    }

    /// @brief Kaybedilen (atılan) log sayacı — teşhis için.
